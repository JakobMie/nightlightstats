  ...
  for (i in 1:length(area_names)){
    ...
    for (j in 1:length(sequence)){
      ...
      if (is.null(fixed_scale_high)){
        fixed_scale_high_plot <- suppressWarnings(max(df_light$light, na.rm = TRUE))
      ...
      plot_lights <- ggplot2::ggplot() +
          ggplot2::geom_tile(data = df_light, mapping = ggplot2::aes(x = lon, y = lat, fill = light), alpha = 0.8) +
          ggplot2::scale_fill_gradientn(limits = c(fixed_scale_low_plot, fixed_scale_high_plot),
                                        colors = plotrix::smoothColors(colours[1], 100, colours[2]), na.value = colours[3]) +
          ggplot2::coord_quickmap() +
          ggplot2::geom_path(data = shapefile, mapping =ggplot2::aes(x = long, y = lat, group = group), colour = colours[4]) +
